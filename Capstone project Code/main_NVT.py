...
dt=0.00001#time period
total_steps=5000 #total steps should be large enough to reach equilibrium
...
T=600 #Temperature for canonical ensemble
vmax=20 #restriction for initialzing velocities
epsilon = -0.0077*(1.602*10**-19) #ε in L-J potential(SI)
...
for n in range(N):
    ...
    for i in range(3):
        vSqd=vSqd+v[n][i]**2
    v_absolute[n]=vSqd**0.5

with open("v_absolute-200-5000.txt", "w") as f:
    for i in range(N):
        f.write(str(v_absolute[i]))
        f.write('\r\n')

with open("vx-200-5000.txt", "w") as f:
    for i in range(N):
        f.write(str(v[i][0]))
        f.write('\r\n')

with open("vy-200-50000.txt", "w") as f:
    for i in range(N):
        f.write(str(v[i][1]))
        f.write('\r\n')

with open("vz-200-5000.txt", "w") as f:
    for i in range(N):
        f.write(str(v[i][2]))
        f.write('\r\n')
